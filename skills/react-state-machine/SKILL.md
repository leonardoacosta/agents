...
description: "Building type-safe, visualizable React state machines with XState v5 and the actor model. Use when the user wants XState-specific implementation — setup() pattern, useMachine/useActorRef, promise actors, parallel states, statechart patterns, XState testing, or migrating from useState/useReducer to XState. Also use when the user explicitly asks for XState or @xstate/react. Do NOT use for general state machine modeling questions (use /state-machine first), AWS Step Functions workflows, or Godot game AI state machines."
