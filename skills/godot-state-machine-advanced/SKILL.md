...
description: "Expert blueprint for hierarchical finite state machines (HSM) and pushdown automata in Godot game engine for complex AI/character behaviors. Covers state stacks, sub-states, transition validation, state context passing, and concurrent machines. Use when the user is building game AI, character controllers, combat systems, or NPC behavior in Godot with layered state machines — HSM, pushdown automata, pushdown stack, Godot state machine, Godot FSM, game AI behavior tree alternative. Do NOT use for web UI state machines (React/XState), cloud workflow orchestration (AWS Step Functions), or general-purpose FSM design — those route to other skills."
---
