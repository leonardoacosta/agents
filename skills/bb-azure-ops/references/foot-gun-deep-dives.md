...
```bash
CF_TOK=$(jq -r '.env.CF_API_TOKEN' ~/dev/brown/wholesale/.claude/settings.local.json)
...
```bash
ZONE_ID=$(jq -r '.zones.stage.id' ~/dev/brown/wholesale/scripts/dns-flip/manifest.json)
