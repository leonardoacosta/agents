...
`~/dev/brown/wholesale/scripts/lib/ado-pipelines.json` — this markdown table is a convenience cache;
...
`~/dev/brown/wholesale/scripts/ado queue` resolves `<code>` + `<env>` → def id via
...
```bash
~/dev/brown/wholesale/scripts/ado registry                          # code|satellite|dev|test|stage|prod table
~/dev/brown/wholesale/scripts/ado queue fb --branch dev             # queue fireball dev (REST + auto env-param)
~/dev/brown/wholesale/scripts/ado queue fb --branch dev --watch     # + stream the timeline as pino NDJSON
~/dev/brown/wholesale/scripts/ado queue dp --branch test --dry-run  # resolve only — prints {code,satellite,env,
                                                       #   pipelineId,ref,yaml,templateParameters}, no queue
~/dev/brown/wholesale/scripts/ado watch <runId> --pipe fb --env dev # monitor a running build (pino events)
```
...
TOK=$(~/dev/brown/wholesale/scripts/ado token)
...
```bash
~/dev/brown/wholesale/scripts/ado raw pipelines create \
...
```bash
grep -nE "repository:|name:|ref:" ~/dev/brown/wholesale/.azuredevops/build/satellites/b3admin.yml | head
...
`~/dev/brown/wholesale/scripts/ado watch <runId> [--pipe <code>] [--env <env>] [--poll <sec=8>] [--pretty]`
...
auto-runs `npm install --prefix ~/dev/brown/wholesale/scripts` once if pino is missing). The NDJSON emits:
...
Live consumer: `~/dev/brown/wholesale/scripts/lib/postman-trace.py` (turns this query into per-endpoint notes).

---

## Postman per-env smoke

`~/dev/brown/wholesale/scripts/bin/postman-smoke <dev|test|stage>` runs the 221-request wholesale
