...
description: >-
  ...
  APIM-WHS-346-*, --as-admin, --as-o365, scripts/ado, scripts/snow-refresh, snow-cookie-refresh,
...
The B&B-specific operational overlay on top of generic Azure knowledge. Two local Azure CLI identities, one bastion, one SNOW host, multiple SPNs with shared limits — this skill encodes how to reach Azure, ADO, Graph, and SNOW resources in the Brown & Brown tenant from a Linux workstation.
...
| `az graph query -q "X \| where ..."` fails: `unrecognized arguments: \| where ...` | Historical wrapper bug: argv used to be word-split before delegation. Phase 3 wrapper preserves argv exactly. | Use the public `az` wrapper normally; if this recurs, fix the wrapper, do not bypass identity/proxy ownership. |
...
| `legacy ALL-Wholesale-*` sub query returns empty | BBAdmin has no RBAC there | Only use O365 for the exact documented non-Graph exception shape, with `--as-o365 --exception` and `AZ_EXCEPTION_CALLER` set by the owning helper. Do not use a generic `account set` recipe. |
...
| A device-code/`az login` re-auth reported success but the failing operation is STILL dead | The wrong identity was refreshed or verification used the wrong surface. | Re-auth through the public wrapper (`az --as-admin login` or `az --as-o365 login`). Verify BBAdmin with a normal ARM/default probe; verify O365 with canonical Graph token traffic. |
| Concurrent agents clobber the local az session | Auth/account operations bypassed the wrapper's per-profile locks. | Use the public `az` wrapper or `scripts/az-reauth.sh`; do not call the real Azure CLI directly or set `AZURE_CONFIG_DIR` in orchestration scripts. |
...
```
Does the URL or token resource target graph.microsoft.com?
  │
  ├─ YES → does the call need Chat.Read (Teams chat content)?
  │         ├─ YES → O365 Graph path (`az --as-o365 account get-access-token --resource https://graph.microsoft.com/` or graph-token helper)
  │         └─ NO  → Graph auto-route via public `az` wrapper unless a documented helper says otherwise
  │
  └─ NO  → does the resource live in an ALL-Wholesale-* (legacy) sub?
          ├─ YES → only the documented non-Graph O365 exception shape (`--as-o365 --exception` + `AZ_EXCEPTION_CALLER`)
          └─ NO  → /home/nyaptor/.local/bin/az ... (bare/default BBAdmin ARM; `--as-admin` optional)
```
...
## Two identities, one wrapper

`~/.local/bin/az` is the controller for B&B Azure CLI calls. It picks the identity by auto-route or flag, preserves argv exactly when delegating to the real Azure CLI, and owns `AZURE_CONFIG_DIR`, SOCKS proxy env vars, per-profile locks, fail-fast markers, and login epoch stamps:

| Flag | UPN | Config Dir | Proxy | Use For |
| --- | --- | --- | --- | --- |
| bare / `--as-admin` (default) | `BBAdminLAcosta@bbins.com` | `~/.azure-bbadmin` | SOCKS5 via cloudpc | Azure ARM in WHS-346 subs, ADO, PIM, Bicep deploys |
| Graph auto-route / `--as-o365` | `leonardo.acosta@bridgespecialty.com` | `~/.azure-o365` | SOCKS5 via cloudpc | Microsoft Graph (Teams, Outlook, OneDrive) |

The wrapper auto-switches to O365 only when the invocation targets `graph.microsoft.com`, including canonical verification via `az --as-o365 account get-access-token --resource https://graph.microsoft.com/`. BBAdmin is the default for bare `az` and non-Graph ARM/ADO commands. Non-Graph O365 is an exact exception boundary only: the owning helper must pass `--as-o365 --exception` and set `AZ_EXCEPTION_CALLER`. Retired or unsupported `--as-*` selectors fail before the real Azure CLI; use a project-owned wrapper for non-B&B Azure access.
...
```bash
az --as-admin login       # BBAdmin, normal ARM/default verification
az --as-o365 login        # O365, verify with canonical Graph token traffic
```
...
| ALL-Wholesale-DEV (legacy) | `979366b2-fe6a-4ee9-a57d-5b280907c375` | documented non-Graph O365 exception only |
| ALL-Wholesale-PROD (legacy) | `0e4f65bb-4da4-49e3-905d-99abb404618d` | documented non-Graph O365 exception only |
| WHS-537-Decus-DEV | `0c3fe2bc-ccf2-4017-90f6-4ebd7df2c9f6` | `--as-admin` (PIM-activate first) |

The legacy `ALL-Wholesale-*` subs are the rare non-Graph O365 boundary because BBAdmin lacks RBAC there. Use only repo-owned helpers that set `AZ_EXCEPTION_CALLER` and pass the exact wrapper exception; do not switch the global account context by hand.
...
```bash
~/dev/brown/wholesale/scripts/ado test                 # SOCKS tunnel + local mint + REST (primary); SSH graded as fallback
~/dev/brown/wholesale/scripts/ado projects             # list all 44 projects
~/dev/brown/wholesale/scripts/ado repos "Wholesale Architecture"
~/dev/brown/wholesale/scripts/ado pipelines Fireball
~/dev/brown/wholesale/scripts/ado runs Fireball        # last 10 pipeline runs
~/dev/brown/wholesale/scripts/ado prs Fireball         # open PRs
~/dev/brown/wholesale/scripts/ado wi PIPS              # work items
~/dev/brown/wholesale/scripts/ado token                # raw AAD token
~/dev/brown/wholesale/scripts/ado registry            # 14 project codes -> pipeline-id table (alias: codes)
~/dev/brown/wholesale/scripts/ado queue fb --branch dev          # queue by code (REST + auto env-param)
~/dev/brown/wholesale/scripts/ado queue fb --branch dev --watch  # + stream the timeline as pino NDJSON
~/dev/brown/wholesale/scripts/ado queue dp --branch test --dry-run  # resolve only (no queue)
~/dev/brown/wholesale/scripts/ado watch <runId> --pipe fb --env dev # monitor a running build (pino)
~/dev/brown/wholesale/scripts/ado <az-devops-args>     # pass-through
```
...
| **`scripts/ado`** | `~/dev/brown/wholesale/scripts/ado` (SOCKS-first: local BBAdmin token mint + ADO REST via :1080, resource `499b84ac-…`; SSH PS proxy only for the extension shortcuts) | `~/dev/brown/wholesale/scripts/ado test` (grades SOCKS primary; SSH reported warn-only) | Committed in the ws repo — `git -C ~/dev/brown/wholesale status scripts/ado`; primary path needs a live local `az --as-admin` session + the tunnel; only PS-proxy shortcuts need the SSH alias + cloudpc `az login` |
| **Graph token (BBAdmin)** | `~/.graph-bbadmin-token.json` (O365 sibling: `~/.graph-token.json`) | refreshed by `~/dev/brown/wholesale/scripts/graph-token-refresh` + `graph-token-refresh.timer` (systemd `--user`); manual: `scripts/graph-token-refresh` (BBAdmin) / `scripts/graph-token-refresh --file ~/.graph-token.json --client <id>` (O365) | File is seeded once from a cloudpc `Connect-MgGraph` mint; if no `refresh_token` in the JSON, re-mint on cloudpc (see `references/graph-endpoints.md`) |
| **`postman-smoke`** | `~/dev/brown/wholesale/scripts/bin/postman-smoke` (newman over the 221-request wholesale collection) | `postman-smoke <dev\|test\|stage>` — see `postman-smoke --help` | Committed in ws; needs `newman` + the env files (see `references/whs-pipelines.md` § Postman per-env smoke) |
| **`scripts/az-audit`** | `~/dev/brown/wholesale/scripts/az-audit` (per-identity ARM health, tunnel state, cache age) | `scripts/az-audit` (exit 0 = all green) — see its header comment | Committed in ws; it only reads, nothing to create |
| **`scripts/snow-refresh`** | `~/dev/brown/wholesale/scripts/snow-refresh` (CDP cookie refresh → `/tmp/snow-cookies.txt`) + `snow-cookie-refresh.timer` | `snow-refresh test` (end-to-end) — see `snow-refresh --help` | First-run needs a one-time interactive SAML sign-in on cloudpc (`snow-refresh start`); full mechanics in `references/snow-mechanics.md` |
...
~/dev/brown/wholesale/scripts/ado test
```
...
4. If the finding is broader than the operational layer (it changes how a satellite is structured, or it's a policy decision), also reflect it in the relevant `~/dev/brown/wholesale/CLAUDE.md` section or project memory
