...
2. Validate each selected change with `openspec validate <id> --strict`. Run the repository dependency validator. In repositories adopting the standard location, run `python3 scripts/openspec/validate_dependencies.py --root . --json <selected-id> ...`; inspect its exit status and JSON. For another repository, use its documented equivalent. If no validator exists, stop admission and request setup; do not substitute a model-guessed graph. Install missing validator dependencies only with appropriate authorization, never silently.
